  ...
  module Filters
    module FormtasticAddons
      ...
      def humanized_method_name
        if object.context.klass.respond_to?(:human_attribute_name)
          object.context.klass.human_attribute_name(method)
        else
      ...
      def reflection_for(method)
        @object.context.klass.reflect_on_association(method) if @object.base.respond_to?(:reflect_on_association)
      end

      # Returns the column for an attribute on the object being searched if it exists.
      def column_for(method)
        @object.context.klass.columns_hash[method.to_s] if @object.base.respond_to?(:columns_hash)
      end
      ...
      def foreign_key?(method)
        @object.context.klass.reflections.select{ |_,r| r.macro == :belongs_to }.values
      ...
      def polymorphic_foreign_type?(method)
        ...
        @object.context.klass.reflections.values.select{ |r| r.macro == :belongs_to && r.options[:polymorphic] }
